...
import { Fingerprint, CheckCircle, AlertCircle, Loader2, Activity, RefreshCcw } from 'lucide-react';
...
export function ScanAssistantPanel({
  ...
  const iconClass = status === 'capturing' || status === 'detecting' ? 'animate-spin' : '';

  return (
    <Card className={`border-2 rounded-xl ${borderColors[status]} ${bgColors[status]} transition-all duration-300`}>
      ...
      <CardContent className="px-4 pb-4 pt-0">
        ...
        <div className="flex justify-center mb-4">
          {previewFrame ? (
            ...
            <div className="motion-safe:block motion-reduce:hidden">
              {animationData ? (
                <Lottie animationData={animationData} loop autoplay className="w-64 h-64" />
              ) : (
        ...
        <div className="text-center space-y-2">
          ...
          {metrics && (Math.abs(metrics.centroid_dx || 0) > 0.1 || Math.abs(metrics.centroid_dy || 0) > 0.1) && status === 'detecting' && (

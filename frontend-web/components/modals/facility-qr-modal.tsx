...
export function FacilityQRModal({ isOpen, onClose, facility }: FacilityQRModalProps) {
  ...
  return (
    <ModalShell
      ...
      backdropClassName="bg-teal-950/60 backdrop-blur-md"
      panelClassName="max-w-xl rounded-[2.5rem] border border-teal-100 flex flex-col"
      showTopBar
      topBarClassName="h-3 bg-gradient-to-r from-teal-400 to-cyan-400 shrink-0"
    >
      <div className="p-7">
        {/* Header */}
        ...
        <div className="bg-teal-50/30 rounded-2xl p-5 mb-6 border border-teal-100/50 min-h-[110px] flex flex-col justify-center">
          <p className="text-sm font-bold text-teal-600/50 uppercase tracking-widest mb-1">
            Facility
          </p>
        ...
        <div className="flex gap-2 mb-6 bg-teal-50/50 p-1.5 rounded-2xl">
          {tabs.map((tab) => (
            ...
            >
              <tab.icon
                className={`w-5 h-5 ${activeTab === tab.id ? "text-teal-600" : "text-teal-600/40"}`}
              />
              {tab.label}
            </button>
          ))}

...
class NoValidImagesError(ImageProcessingError):
    ...
    def __init__(self):
        super().__init__(
            message="No valid fingerprint images provided",
            status_code=400
        )
...
class InvalidAgeError(ValidationError):
    ...
    def __init__(self, age: int):
        super().__init__(
            field="age",
            message=f"Age must be between 1 and 120. Got: {age}"
        )
...
class InvalidBMIError(ValidationError):
    ...
    def __init__(self, weight: float, height: float):
        super().__init__(
            field="bmi",
            message=f"Invalid BMI for weight={weight}kg, height={height}cm"
        )
